...
CREATE TABLE IF NOT EXISTS report_transition_history (id BIGSERIAL PRIMARY KEY,report_id INTEGER NOT NULL REFERENCES reports(id),actor_id INTEGER REFERENCES users(id),actor_role VARCHAR(32),from_status VARCHAR(40),to_status VARCHAR(40) NOT NULL,reason TEXT,decision_data JSONB NOT NULL DEFAULT '{}'::JSONB,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
CREATE TABLE IF NOT EXISTS report_proofs (id BIGSERIAL PRIMARY KEY,report_id INTEGER NOT NULL REFERENCES reports(id),agent_id INTEGER NOT NULL REFERENCES users(id),media_asset_id UUID NOT NULL REFERENCES media_assets(id),captured_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),latitude DOUBLE PRECISION,longitude DOUBLE PRECISION,accuracy_m DOUBLE PRECISION,comment VARCHAR(1000),status VARCHAR(32) NOT NULL DEFAULT 'soumise' CHECK(status IN ('soumise','en_examen','acceptee','refusee','remplacement_demande')),decided_by INTEGER REFERENCES users(id),decision_reason VARCHAR(1000),decided_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
CREATE TABLE IF NOT EXISTS report_disputes (id BIGSERIAL PRIMARY KEY,report_id INTEGER NOT NULL REFERENCES reports(id),citizen_id INTEGER NOT NULL REFERENCES users(id),reason VARCHAR(64) NOT NULL,comment VARCHAR(2000) NOT NULL,media_asset_id UUID REFERENCES media_assets(id),status VARCHAR(24) NOT NULL DEFAULT 'soumise' CHECK(status IN ('soumise','en_examen','confirmee','reouverte','cloturee')),created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),resolved_at TIMESTAMPTZ);
CREATE TABLE IF NOT EXISTS assignment_history (id BIGSERIAL PRIMARY KEY,resource_type VARCHAR(20) NOT NULL CHECK(resource_type IN ('report','collection')),resource_id INTEGER NOT NULL,assignee_id INTEGER REFERENCES users(id),actor_id INTEGER REFERENCES users(id),action VARCHAR(24) NOT NULL CHECK(action IN ('assigned','reassigned','accepted','refused','released')),reason VARCHAR(1000),created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
CREATE TABLE IF NOT EXISTS collection_transition_history (id BIGSERIAL PRIMARY KEY,occurrence_id INTEGER NOT NULL REFERENCES collection_occurrences(id),actor_id INTEGER REFERENCES users(id),actor_role VARCHAR(32),from_status VARCHAR(40),to_status VARCHAR(40) NOT NULL,reason VARCHAR(1000),created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
CREATE TABLE IF NOT EXISTS job_runs (id BIGSERIAL PRIMARY KEY,job_name VARCHAR(80) NOT NULL,idempotency_key VARCHAR(160) NOT NULL UNIQUE,status VARCHAR(20) NOT NULL CHECK(status IN ('running','completed','failed')),result JSONB NOT NULL DEFAULT '{}'::JSONB,started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),finished_at TIMESTAMPTZ);

-- Advanced operational mapping (migration 008).
...
CREATE TABLE IF NOT EXISTS tour_stops (id BIGSERIAL PRIMARY KEY,tour_id INTEGER NOT NULL REFERENCES tours(id),report_id INTEGER REFERENCES reports(id),occurrence_id INTEGER REFERENCES collection_occurrences(id),stop_order INTEGER NOT NULL CHECK(stop_order>0),status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','current','completed','missed','cancelled')),geometry GEOMETRY(Point,4326) NOT NULL,completed_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),CHECK((report_id IS NOT NULL)::int+(occurrence_id IS NOT NULL)::int=1),CHECK(ST_SRID(geometry)=4326),UNIQUE(tour_id,stop_order),UNIQUE(tour_id,report_id),UNIQUE(tour_id,occurrence_id));
...
CREATE TABLE IF NOT EXISTS route_versions (id BIGSERIAL PRIMARY KEY,tour_id INTEGER NOT NULL REFERENCES tours(id),version INTEGER NOT NULL,provider VARCHAR(32),status VARCHAR(20) NOT NULL CHECK(status IN ('ready','unavailable','failed')),geometry GEOMETRY(LineString,4326),distance_m DOUBLE PRECISION,duration_s DOUBLE PRECISION,error_code VARCHAR(80),created_by INTEGER REFERENCES users(id),created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),UNIQUE(tour_id,version));
CREATE TABLE IF NOT EXISTS operational_positions (id BIGSERIAL PRIMARY KEY,tour_id INTEGER NOT NULL REFERENCES tours(id),actor_id INTEGER NOT NULL REFERENCES users(id),actor_role VARCHAR(20) NOT NULL CHECK(actor_role IN ('agent','ramasseur')),geometry GEOMETRY(Point,4326) NOT NULL,accuracy_m DOUBLE PRECISION NOT NULL CHECK(accuracy_m>=0 AND accuracy_m<=10000),source VARCHAR(20) NOT NULL DEFAULT 'device',recorded_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),expires_at TIMESTAMPTZ NOT NULL,CHECK(ST_SRID(geometry)=4326),CHECK(expires_at>recorded_at));
...
CREATE TABLE IF NOT EXISTS tour_incidents (id BIGSERIAL PRIMARY KEY,tour_id INTEGER NOT NULL REFERENCES tours(id),stop_id BIGINT REFERENCES tour_stops(id),author_id INTEGER NOT NULL REFERENCES users(id),category VARCHAR(64) NOT NULL,description VARCHAR(2000) NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'open' CHECK(status IN ('open','resolved')),created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),resolved_at TIMESTAMPTZ);
